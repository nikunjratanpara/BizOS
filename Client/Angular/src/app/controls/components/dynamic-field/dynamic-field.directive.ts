...
import { IField, IDynamicField } from '../../models/field.interface';
...
export class DynamicFieldDirective implements IDynamicField, OnInit, OnChanges {
  @Input()
  config: IFieldConfig;
  ...
  ngOnInit() {
    ...
    const component = this.resolver.resolveComponentFactory<IField>(components[this.config.type]);
  ...
  private assignComponentConfig() {
    ...
    this.component.instance.validation = this.config.validation;
